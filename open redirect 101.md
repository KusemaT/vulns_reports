...
Open Redirect via Visit page Parameter on Organic Keyword Rankings Page
...
Semrush https://admin-out.kzlabs.in/ visit page parameter is vulnerable to an open redirect.
The website redirects users to an external URL supplied via the visit page parameter without validating that the destination belongs to a trusted/allow-listed domain. An attacker can craft a link that appears to point to the legitimate site but silently redirects the victim to an attacker-controlled site, which is commonly abused for phishing and credential theft.

## Vulnerable Endpoint

`https://admin-out.kzlabs.in/`

## Steps to Reproduce

1. Navigate to the following URL: `https://admin-out.kzlabs.in/`
2. Capture re-direct request in burp suite
3. Send Request to burp suite
4. Brute-force by changing payloads to existing links
5. Observe that the browser is redirected away from the trusted domain to the attacker-controlled domain (`evil.example`) without any warning
...
<img width="1792" height="1120" alt="image" src="https://github.com/user-attachments/assets/67e6bae0-b517-4153-bb10-ed5a31dcf17c" />


`Screenshot 2: Screenshot shows the browser being redirected to the attacker-controlled domain`

<img width="1792" height="1120" alt="image" src="https://github.com/user-attachments/assets/b1343754-d9ba-4e14-89bb-f4b39d97dbb3" />
