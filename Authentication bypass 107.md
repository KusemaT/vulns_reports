...
Authentication Bypass via Parameter Manipulation on https://kzlabs.in/subdomains/adminpanel/
...
## Summary
Admin Panel https://kzlabs.in/subdomains/adminpanel/ is vulnerable to an authentication bypass.
The application fails to properly enforce authentication checks on status of request parameter, allowing an attacker to access authenticated functionality or another user's account without providing valid credentials.

## Vulnerable Endpoint

`https://kzlabs.in/subdomains/adminpanel/` 

## Steps to Reproduce

1. Log out, or open the target endpoint in an unauthenticated/incognito session
2. Navigate directly to the following URL: `https://kzlabs.in/subdomains/adminpanel/`
   (Alternatively: intercept the login/authenticated request in Burp Suite and [e.g. modify the `status` parameter, remove the `message` header, replay a request with a manipulated status of :true)
3. Observe that the application grants access to protected content/functionality without requiring valid authentication

## Payload used

`modified request status request to true`

## Proof of Concept Request

`Screenshot 1: Screenshot shows the request sent without valid authentication`

<img width="1792" height="1120" alt="image" src="https://github.com/user-attachments/assets/a1afaf30-88f0-4266-ac09-e972fdeeaa28" />
...
<img width="940" height="1099" alt="image" src="https://github.com/user-attachments/assets/64a4af25-5943-4af6-aa48-4af7cbed4c16" />
